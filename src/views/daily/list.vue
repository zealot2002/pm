<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="120px" align="center" label="模块">
        <template slot-scope="scope">
          <span>{{ getModuleName(scope.row.mid) }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="dialogStatus === 'create' ? '添加日报' : '编辑日报'" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item label="项目" prop="pid">
          <el-select v-model="temp.pid" placeholder="请选择项目" clearable @change="handleTempProjectChange">
            <el-option v-for="item in projectOptions" :key="item.id" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="模块" prop="mid">
          <el-select v-model="temp.mid" placeholder="请选择模块" clearable>
            <el-option v-for="item in moduleOptions" :key="item.id" :label="item.modelname" :value="item.id" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getModules(pid) {
      ...
      getModelList({ pid: pid }).then(response => {
        if (response.code === 1) {
          this.moduleOptions = response.data.data
          // 创建模块ID到名称的映射
          this.moduleOptions.forEach(item => {
            this.moduleMap[item.id] = item.modelname
          })
        } else {
    ...
    getModuleName(id) {
      return id && this.moduleMap[id] ? this.moduleMap[id] : '未指定'
    },
    ...
    handleProjectChange() {
      if (this.listQuery.pid) {
        this.getModules(this.listQuery.pid)
      }
      this.listQuery.page = 1
      this.getList()
    },
    
    handleTempProjectChange() {
      if (this.temp.pid) {
        this.getModules(this.temp.pid)
      } else {
        this.moduleOptions = []
      }
      this.temp.mid = undefined
    },
    ...
    handleUpdate(row) {
      ...
      // 确保模块选项已加载
      if (this.temp.pid && (!this.moduleOptions.length || this.moduleOptions[0].pid !== this.temp.pid)) {
        this.getModules(this.temp.pid)
      }
